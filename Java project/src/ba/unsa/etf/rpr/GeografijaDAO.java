...
public class GeografijaDAO {
    ...
    private PreparedStatement glavniGradUpit, dajDrzavuUpit, obrisiDrzavuUpit, obrisiGradoveZaDrzavuUpit, nadjiDrzavuUpit,
            ...
            nadjiGradUpit, obrisiGradUpit, dajDrzaveUpit, odrediIdRegijeUpit, dajRegijeUpit, dodajRegijuUpit;
    ...
    private GeografijaDAO() {
        ...
        try {
            ...
            dajRegijeUpit = conn.prepareStatement("SELECT * FROM regija ORDER BY naziv");
            ...
            odrediIdRegijeUpit = conn.prepareStatement("SELECT MAX(id) FROM regija");
            dodajRegijuUpit = conn.prepareStatement("INSERT INTO regija VALUES(?,?)");
    ...
    public void vratiBazuNaDefault() throws SQLException {
        ...
        stmt.executeUpdate("DELETE FROM regija");
    ...
    private Regija dajRegijuIzResultSeta(ResultSet rs) throws SQLException {
        Regija r =  new Regija(rs.getInt(1), rs.getString(2));
        return r;
    }
    ...
    public void dodajRegiju(Regija regija) {
        try {

            ResultSet rs = odrediIdRegijeUpit.executeQuery();
            int id = 1;
            if (rs.next()) {
                id = rs.getInt(1) + 1;
            }

            dodajRegijuUpit.setInt(1, regija.getId());
            dodajRegijuUpit.setString(2, regija.getNaziv());

            dodajRegijuUpit.executeUpdate();
    ...
    public ArrayList<Regija> regije() {
        ArrayList<Regija> rezultat = new ArrayList();
        try {
            ResultSet rs = dajRegijeUpit.executeQuery();
            while (rs.next()) {
                Regija regija = dajRegijuIzResultSeta(rs);
                rezultat.add(regija);
            }
        } catch (SQLException e) {
            e.printStackTrace();
